...
class EntityLinker:
    def __init__(self):
        ...
        try:
            self.df_qa = pd.read_csv(qa_file_path,sep='\t', header=None, names= ['question_sentence','answer_set','answer_path'])
            ...
            self.df_kb = pd.read_csv(kb_file_path,sep='\s', header=None, names= ['e_subject','relation','e_object'])
            self.create_entity_set()
        except Exception as e:
            print('PQ-2H.txt or 2H-kb.txt file path wrong')
