...
def prep_dataset(path_KB, path_QA):
    ...
    '''
    # get KG
    df_graph = pd.read_csv(path_KB, sep=r'\s', header=None, names=[
                           'e_subject', 'relation', 'e_object'])
    KB = nx.from_pandas_edgelist(
        df_graph, "e_subject", "e_object", edge_attr="relation", create_using=nx.DiGraph())

    # get questions
    df_qn = pd.read_csv(path_QA, sep=r'\t', header=None, names=[
                        'question_sentence', 'answer_set', 'answer_path'])
    df_qn['answer'] = df_qn['answer_set'].apply(lambda x: x.split('(')[0])
    df_qn['q'] = df_qn['question_sentence'].apply(lambda x: x.split(' '))

    # TODO replace e_s with jiaheng's thing

    qn_list = df_qn[['q', 'answer']].values.tolist()
