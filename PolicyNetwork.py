...
class PolicyNetwork():
    ...
    def run_train_op(self, train_set, predictions = False):
        ...
        preds_actions_probs_discounts_r = tf.map_fn(lambda data: self.forward(data), train_set.eval(session=self.sess))
        predictions, actions_probs_discounts_rs = tf.unstack(preds_actions_probs_discounts_r, axis=1).eval(session=self.sess)
        pass
